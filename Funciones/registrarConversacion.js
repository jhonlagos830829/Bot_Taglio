...
const { ctx } = require('../FlujosBotTaglio/flujoPrecios');

async function guardarEnBaseDatos(data, tablaBaseDatos){
  ...
  try {
    ...
    let config = {
      ...
      url: configuracion.URL_API + tablaBaseDatos,
      headers: { 
...
async function actualizarEnBaseDatos(data, tablaBaseDatos){
...
async function guardarConversacion(ctx, mensajeBot){
  ...
  try {
    ...
    this.guardarEnBaseDatos(data, 'lb-conversacions')
...
async function guardarCita(ctx, nombreCliente, fecha, idCalendarioCita, estado){
  ...
  try {
    ...
    //Guardar en la base de datos
    this.guardarEnBaseDatos(data, 'lb-citas')
...
async function guardarSolicitudNotificacion(ctx){
  ...
  try {
    ...
    let data = JSON.stringify({
      "data": {
        ...
        "motivo": "Agenda disponible",
        "notificado": false
      }
    });

    //Guardar en la base de datos
    this.guardarEnBaseDatos(data, configuracion.TABLA_SOLICITUDES_NOTIFICACION_CLIENTES)
...
async function actualizarSolicitudNotificacion(idSolicitud){
  ...
  try {
    ...
    //Guardar en la base de datos
    this.actualizarEnBaseDatos(data, configuracion.TABLA_SOLICITUDES_NOTIFICACION_CLIENTES + "/" + idSolicitud)
...
async function obtenerCita(idCalendarioCita){
  ...
  try {
    ...
    */
    ...
    console.log('Los datos de la cita a eliminar -> ' + datosCita)
...
async function obtenerSolicitudesNotificacion(){
  ...
  try {
    ...
    */
    ...
    //Guardar en la base de datos
    datosCita = await this.buscarEnBaseDatos(configuracion.URL_OBTENER_NOTIFICACIONES_PENDIENTES)
...
async function registrarConversacion(NombreBot, idConversacion, nombreWhatsappCliente, telefonoCliente, mensajeBot, mensajeCliente) {
...
async function registrarMovimientoCuenta(codigoCuentaMovimiento, conceptoMovimiento, fechaMovimiento, tipoMovimiento, origenMovimiento, destinoMovimiento, notaMovimiento, valorMovimiento, referenciaMovimiento) {
...
module.exports.guardarSolicitudNotificacion = guardarSolicitudNotificacion;
module.exports.actualizarSolicitudNotificacion =actualizarSolicitudNotificacion;
module.exports.actualizarEnBaseDatos = actualizarEnBaseDatos;
module.exports.obtenerSolicitudesNotificacion = obtenerSolicitudesNotificacion;
