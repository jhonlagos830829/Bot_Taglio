...
module.exports = flujoCrearCita = addKeyword(ExpRegFlujo, { regex: true })
    ...
    .addAction(async (ctx, ctxFn) => {
        ...
        let fechas_citas = ctxFn.state.get('fecha_cita_cliente')
        ...
        try {
            ...
            //Recorrer las fechas de las citas
            for(const fechaCita of fechas_citas.split(',')){
                
                //Crear la cita y almacenar el resultado
                let citaCalendario = await calendarioGoogle.crearCita(/*'Corte para ' + */cliente_cita + ' ' + ctx.from, fechaCita)

                //Registrar la cita en la base de datos
                await cita.Guardar(ctx, cliente_cita, fechaCita, citaCalendario.data.id, 'Pendiente')

                //Enviar mensaje de notificación de nueva cita creada
                await ctxFn.provider.sendText(configuracion.NUMERO_NOTIFICAR_CITA, mensajes.MENSAJE_NOTIFICACION_NUEVA_CITA + '\n\n' + '*Fecha:* ' + fechas.fechaHoraLegible(fechaCita) + '\n*Cliente:* ' + cliente_cita)
                ...
                await ctxFn.flowDynamic(mensajes.MENSAJE_RADICADO_FLUJO_CREAR_CITA)

                //Realizar una pausa de 1 segundo
                await delay(1000)

                //Enviar mensaje de confirmación al cliente
                await ctxFn.flowDynamic(citaCalendario.data.id)
                ...
                //Enviar mensaje de confirmación al cliente
                await ctxFn.flowDynamic(mensajes.MENSAJE_NO_PIERDA_RADICADO_FLUJO_CREAR_CITA)
